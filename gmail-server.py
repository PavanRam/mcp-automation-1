...
class GmailService:
    ...
    # FIX: Added attachment_path as an optional parameter
    async def send_email(self, recipient_id: str, subject: str, message: str, attachment_path: str | None = None) -> dict:
        """Creates and sends an email message with optional attachment"""
        try:
            ...
            # ATTACHMENT LOGIC
            if attachment_path:
                path = pathlib.Path(attachment_path)
                if path.exists():
                    # Guess the MIME type
                    ...
                    with open(path, 'rb') as fp:
                        file_data = fp.read()
                        message_obj.add_attachment(file_data,
                                                   maintype=main_type,
                                                   subtype=sub_type,
                                                   filename=path.name)
                    logger.info(f"Attached file: {path.name}")
                else:
                    return {"status": "error", "error_message": f"Attachment file not found: {attachment_path}"}
...
async def main(creds_file_path: str,
    ...
    async def handle_list_tools() -> list[types.Tool]:
        return [
            types.Tool(
                ...
                inputSchema={
                    ...
                    "properties": {
                        ...
                        "attachment_path": { # Included here as optional to allow single tool schema (even if it's best practice to use the specific attachment tool)
                            "type": "string",
                            "description": "Optional absolute path to file to attach",
                        },
                    },
            ...
            types.Tool(
                ...
                description="""Sends email with an attachment to recipient.
                Do not use if user only asked to draft email.
                Drafts must be approved before sending.
                Attachment path must be an absolute path to an existing file.""",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "recipient_id": {
                        ...
                        "attachment_path": {
                            "type": "string",
                            "description": "Absolute path to file to attach",
                        },
                    },
    ...
    async def handle_call_tool(
        ...
        # Unified logic for both send-email and send-email-with-attachment
        if name in ["send-email", "send-email-with-attachment"]:
            if not arguments:
                raise ValueError("Missing arguments for send-email tool")
            ...
            attachment_path = arguments.get("attachment_path", None) # Default to None

            # Extract subject and message content
            email_lines = message.split('\n')
            if email_lines[0].startswith('Subject:'):
                subject_extracted = email_lines[0][8:].strip()
                message_content = '\n'.join(email_lines[1:]).strip()
                # Use the extracted subject if it's there, but fall back to the provided subject if the prompt didn't include it in the message body
                subject = subject_extracted if subject_extracted else subject
            else:
                message_content = message
            
            # FIX: Call the service with all arguments, including the now-optional attachment_path
            send_response = await gmail_service.send_email(recipient, subject, message_content, attachment_path)
